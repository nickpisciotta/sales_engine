class Item < ApplicationRecord
  ...
  def self.items_with_greatest_revenue(quantity_params)
    item_revenue_hash = Item.joins(:invoice_items)
                            .group("items.id")
                            .order("sum_invoice_items_quantity_all_invoice_items_unit_price DESC").limit(quantity_params)
  ...
  def self.items_with_greatest_quantity(quantity_params)
    item_quantity_hash = Item.joins(:invoice_items)
                        .group("items.id")
  ...
  def best_day
    best_day_hash = invoice_items.joins(:invoice)
